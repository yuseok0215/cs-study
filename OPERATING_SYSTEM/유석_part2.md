...
멀티 스레드 프로그래밍에서 일반적으로 어떤 함수나 변수, 혹은 객체가 여러 스레드로부터 동시에 접근이 이루어져도 프로그램의 실행에 문제가 없는 것을 말합니다.

> 12-1. Thread Safe 를 보장하기 위해 어떤 방법을 사용할 수 있나요?

여러가지 방법이 있습니다. 첫 번째로 공유 자원에 하나의 Thread만 접근할 수 있도록 세마포어, 뮤텍스로 락을 통제하는 방법입니다. 두번째로 공유 자원 변경에 필요한 연산을 원자적으로 분리한 뒤에 실제로 데이터의 변경이 이루어지는 시점에 락을 걸고, 다른 스레드의 접근을 막는 방법입니다.

> 12-2. Peterson's Algorithm 이 무엇이며, 한계점에 대해 설명해 주세요.

피터슨 알고리즘은 두 개의 프로세스가 임계구역에 동시에 접근하지 못하도록 보장하는 동기화 알고리즘입니다. 피터슨 알고리즘은 flag와 turn이라는 공유 변수를 사용하여 각 프로세스가 임계구역에 들어갔는지, 임계구역을 어떤 프로세스에 우선적으로 허용하는지를 설정하여 두 프로세스가 상호 배제를 유지할 수 있게 해줍니다.

피터슨 알고리즘의 한계점은 두 개의 프로세스만 지원하기 때문에 다중 프로세스 환경에서는 적용이 어렵다는 점입니다. 그리고 멀티코어 시스템에서는 메모리 일관성 문제로 인해 제대로 작동하지 않을 수 있습니다.

> 12-3. Race Condition 이 무엇인가요?

ace Condition이란 두 개 이상의 프로세스가 공통 자원을 병행적으로 읽거나 쓰는 동작을 할 때, 공용 데이터에 대한 접근이 어떤 순서로 이루어졌는지에 따라 그 실행 결과가 같지 않고 달라지는 상황을 말합니다. 즉, 두 개의 스레드가 하나의 자원을 놓고 서로 사용하려고 경쟁하는 상황을 말합니다.

> 12-4. Thread Safe를 구현하기 위해 반드시 락을 사용해야 할까요? 그렇지 않다면, 어떤 다른 방법이 있을까요?

다른 대안으로 여러가지 방법이 있습니다. 원자적 연산을 이용하여 락 없이 스레드 안정성을 보장할 수 있으며, 불변 객체를 통해 상태 변경을 아예 막는 방법도 있습니다.

제가 주로 사용하는 자바에서는 동시성 컬렉션을 활용하거나 스레드 로컬을 사용하여 스레드 세이프한 상태를 유지하는 방법이 있습니다.
